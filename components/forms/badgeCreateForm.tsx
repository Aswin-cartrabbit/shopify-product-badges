...
import {
  ...
  BadgeAlignment,
} from "@/components/BadgeHorizontalPosition";
...
export const BadgeBuilder = ({
    ...
    if (selectedTemplate) {
      setFormData((prev) => ({
        ...
        name:
          ...
          selectedTemplate.alt ||
          `New ${componentType.toLowerCase()}`,
        design: selectedTemplate,
      }));
      setName(
        selectedTemplate.text ||
  ...
  useEffect(() => {
    if (type === "LABEL") {
      ...
      updateDesign(
        "horizontalPosition",
        BadgeHorizontalPosition.BELOW_PRODUCT_TITLE
      );
    ...
    if (selectedTemplate && !hasInitializedTemplate) {
      ...
      if (selectedTemplate.text && !selectedTemplate.src) {
        ...
        if (selectedTemplate.style) {
          ...
          if (
            selectedTemplate.style.background ||
          ...
          if (selectedTemplate.style.borderRadius) {
            const radius =
              typeof selectedTemplate.style.borderRadius === "string"
                ? parseInt(
                    selectedTemplate.style.borderRadius.replace("px", "")
                  )
          ...
          if (selectedTemplate.style.clipPath) {
            updateDesign(
              "shape",
              `clip-path: ${selectedTemplate.style.clipPath}`
            );
          }
    ...
    try {
      ...
      const payload = {
        ...
        design: {
          ...
          background:
            badge.design.background || formData.design?.background || "solid",
          ...
          cornerRadius:
            badge.design.cornerRadius || formData.design?.cornerRadius || 8,
          borderSize:
            badge.design.borderSize || formData.design?.borderSize || 0,
          borderColor:
            badge.design.borderColor ||
            formData.design?.borderColor ||
            "#c5c8d1",
          ...
          gridPosition:
            badge.design.gridPosition || formData.design?.gridPosition,
          horizontalPosition:
            badge.design.horizontalPosition ||
            formData.design?.horizontalPosition,
          textAlignment:
            badge.design.textAlignment || formData.design?.textAlignment,
          isBold: badge.design.isBold || formData.design?.isBold || false,
          isItalic: badge.design.isItalic || formData.design?.isItalic || false,
          isUnderline:
            badge.design.isUnderline || formData.design?.isUnderline || false,
          spacing: badge.design.spacing ||
            formData.design?.spacing || {
              insideTop: "16",
              insideBottom: "16",
              outsideTop: "20",
              outsideBottom: "20",
        ...
        display: {
          ...
          respectProductPriority:
            badge.display.respectProductPriority !== undefined
              ? badge.display.respectProductPriority
              : true,
          ...formData.display,
        },
        settings: {
